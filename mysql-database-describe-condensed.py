...
def getColumns(mysql, database):
	...
	sql = 'SELECT * FROM `COLUMNS` WHERE `TABLE_SCHEMA` = "%s" ORDER BY `TABLE_NAME`, `ORDINAL_POSITION`' % mysql.escape_string(database)
	return dbQuery(mysql, sql)

# ----------------------------------------------------------------------------
def getKeys(mysql, database):

	sql = 'SELECT * FROM `STATISTICS` WHERE `TABLE_SCHEMA` = "%s" ORDER BY `TABLE_NAME`, `INDEX_NAME`, `SEQ_IN_INDEX`' % mysql.escape_string(database)
	return dbQuery(mysql, sql)

# ----------------------------------------------------------------------------
def getExt(mysql, database):

	sql = 'SELECT * FROM `KEY_COLUMN_USAGE` WHERE `TABLE_SCHEMA` = "%s" AND `REFERENCED_TABLE_SCHEMA` IS NOT NULL ORDER BY `TABLE_NAME`, `CONSTRAINT_NAME`' % mysql.escape_string(database)
	return dbQuery(mysql, sql)
...
def showDetails(tables, columns, keys, ext):
	...
	details = [
		{ 'data': columns, 'names': [ 'TABLE_NAME', 'COLUMN_NAME', 'COLUMN_TYPE', 'COLLATION_NAME', 'COLUMN_KEY', 'EXTRA', 'COLUMN_DEFAULT', 'IS_NULLABLE', 'COLUMN_COMMENT' ] },
		{ 'data': keys, 'names': [ 'TABLE_NAME', 'INDEX_NAME', 'SEQ_IN_INDEX', 'COLUMN_NAME', 'NON_UNIQUE', 'NULLABLE', 'INDEX_TYPE' ] },
		{ 'data': ext, 'names': [ 'TABLE_NAME', 'CONSTRAINT_NAME', 'COLUMN_NAME', 'REFERENCED_TABLE_NAME', 'REFERENCED_COLUMN_NAME' ] }
	]
