...
config = {
	...
	'css': 'body { font-family: Courier New, monospace; font-size: 12px; } h1, h2 { margin: 1em 0; padding: 0; } table { margin: 2em 0; padding: 0; width: 100%; border-collapse: collapse; border-spacing: 0; } table, th, td { border: 1px solid #bbb; } tr:nth-child(even) { background: #fff; } tr:nth-child(odd) { background: #eee; } th, td { padding: 4px; font-size: 14px; } th { color: #fff; background: #444; } td { color: #000; }',
	'enum_split': {
		'from': "','",
		'to': "', '"
	}
...
def showDatabase(database):
	...
	myPrint('<h1>DATABASE: %s</h1>' % database)
...
def showDetails(tables, columns, keys, ext):
	...
	for table in tables:
		...
		myPrint('<h2>TABLE: %s</h2>' % table_name)
		...
		for detail in details:
			...
			for arr in data:
				if arr['TABLE_NAME'] == table_name:
					row = []
					for name in names:
						cel = '' if arr[name] == None else str(arr[name])
						if cel.find(config['enum_split']['from']):
							cel = cel.replace(config['enum_split']['from'], config['enum_split']['to'])
						row.append(cel)
					output.append(row)
...
def createReport(args):
	...
	database = args[4]
	footer = '%s @ %s :: %s' % (args[2], args[1], database)
	...
	myPrint('<html><head><meta charset="' + config['charset'] + '"><title>' + database + '</title><style>' + config['css'] + '</style></head><body>')

	showDatabase(database)
	showTables(tables)
	showDetails(tables, columns, keys, ext)

	myPrint('<h3>' + footer + '</h3></body></html>')
